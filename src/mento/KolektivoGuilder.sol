...
import {CalledByVm} from "./lib/CalledByVm.sol";
import {FixidityLib} from "./lib/FixidityLib.sol";
import {ICeloToken} from "./lib/ICeloToken.sol";
import {ICeloVersionedContract} from "./lib/ICeloVersionedContract.sol";
import {IStableToken} from "./lib/IStableToken.sol";
import {UsingPrecompiles} from "./lib/UsingPrecompiles.sol";
import {UsingRegistry} from "./lib/UsingRegistry.sol";
import {Freezable} from "./lib/Freezable.sol";
    ...
    ICeloToken
{
    ...
    event ExchangeIdentifierUpdated(bytes32 oldId, bytes32 newId);

    event TransferComment(string comment);

    bytes32 constant GRANDA_MENTO_REGISTRY_ID = keccak256(abi.encodePacked("GrandaMento"));
    bytes32 constant KOLEKTIVO_RESERVER_REGISTRY_ID = keccak256(abi.encodePacked("KolektivoCuracaoReserve"));
    ...
    function initialize(
        ...
        require(bytes(name_).length == 0, "Already initialized");
    ...
    function setExchangeRegistryId(string calldata exchangeIdentifier) external onlyOwner {
        bytes32 newId = keccak256(abi.encodePacked(exchangeIdentifier));

        if (newId != exchangeRegistryId) {
            emit ExchangeIdentifierUpdated(exchangeRegistryId, newId);
            exchangeRegistryId = keccak256(abi.encodePacked(exchangeIdentifier));
        }
    }
    ...
    function mint(address to, uint256 value) external updateInflationFactor returns (bool) {
        require(
            msg.sender == registry.getAddressForOrDie(getExchangeRegistryId()) 
                || (totalSupply_ == 0 && msg.sender == registry.getAddressFor(KOLEKTIVO_RESERVER_REGISTRY_ID))
    ...
    function _mint(address to, uint256 value) private returns (bool) {
        ...
        totalSupply_ = totalSupply_ + units;
    ...
    function _burn(address from, uint256 value) internal returns (bool) {
        uint256 units = _valueToUnits(inflationState.factor, value);
        require(units <= balances[from], "value exceeded balance of sender");
        totalSupply_ = totalSupply_ - units;
        balances[from] = balances[from] - units;
        emit Transfer(from, address(0), units);
        return true;
    }
    ...
    function getUpdatedInflationFactor() private view returns (FixidityLib.Fraction memory, uint256) {
        ...
        if (block.timestamp < inflationState.factorLastUpdated + inflationState.updatePeriod) {
        ...
        uint256 timesToApplyInflation =
            (block.timestamp - inflationState.factorLastUpdated) / inflationState.updatePeriod;
        ...
        uint256 lastUpdated = inflationState.factorLastUpdated + (inflationState.updatePeriod * timesToApplyInflation);
    ...
    function creditGasFees(
        ...
        units = units + _creditGas(from, communityFund, baseTxFee);
        units = units + _creditGas(from, feeRecipient, tipTxFee);
        units = units + _creditGas(from, gatewayFeeRecipient, gatewayFee);
        totalSupply_ = totalSupply_ + units;
    }
