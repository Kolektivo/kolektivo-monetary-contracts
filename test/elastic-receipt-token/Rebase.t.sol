 ...
 */
contract ElasticReceiptTokenRebase is ElasticReceiptTokenTest {
    struct TestCase {
        // The user balances.
        uint256[2] balances;
        // The new supply target for the rebase operation.
        uint256 newSupplyTarget;
    }
    ...
    function testRebaseTables() public {
        ...
        TestCase[5] memory testTable = [
            // Equilibrium rebase.
            TestCase([uint256(1e18), 1e18], 2e18),
            // Rebase from min to max.
            TestCase([uint256(1), 1], MAX_SUPPLY),
            // Max supply in one user with min rebase.
            TestCase([uint256(MAX_SUPPLY - 1), 1], 1),
            // Max supply in one user with max rebase.
            TestCase([uint256(MAX_SUPPLY - 1), 1], MAX_SUPPLY),
            // Max supply splitted with min rebase.
            TestCase([uint256(MAX_SUPPLY / 2), MAX_SUPPLY / 2], 1)
        ];

        // Execute each test case.
        for (uint256 i; i < testTable.length; i++) {
            TestCase memory testCase = testTable[i];
            uint256[2] memory balances = testCase.balances;
    ...
    function testNoRebaseIfZeroSupplyTarget() public {
        ...
        uint256 supplyBefore = ert.totalSupply();
        assertEq(supplyBefore, 10e18);

        underlier.burn(address(ert), 10e18);
        ert.rebase();

        uint256 supplyAfter = ert.totalSupply();
    ...
    function setUpUserBalance(address user, uint256 balance) public {
