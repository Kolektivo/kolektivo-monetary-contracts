...
library Errors {
    ...
    bytes internal constant InvalidAmount = abi.encodeWithSignature("InvalidAmount()");
    bytes internal constant InvalidRecipient = abi.encodeWithSignature("InvalidRecipient()");

    function ERC20IsNotBondable(address erc20) internal pure returns (bytes memory) {
        return abi.encodeWithSignature("Treasury__ERC20IsNotBondable(address)", erc20);
    }

    function ERC20IsNotRedeemable(address erc20) internal pure returns (bytes memory) {
        return abi.encodeWithSignature("Treasury__ERC20IsNotRedeemable(address)", erc20);
    }

    function ERC20IsNotRegistered(address erc20) internal pure returns (bytes memory) {
        return abi.encodeWithSignature("Treasury__ERC20IsNotRegistered(address)", erc20);
    }

    function StalePriceDeliveredByOracle(address erc20, address oracle) internal pure returns (bytes memory) {
        return abi.encodeWithSignature("Treasury__StaleERC20PriceDeliveredByOracle(address,address)", erc20, oracle);
    }
}
...
abstract contract TreasuryTest is Test {
    ...
    event ERC20Registered(
        address indexed erc20, address indexed oracle, Treasury.AssetType assetType, Treasury.RiskLevel riskLevel
    );
